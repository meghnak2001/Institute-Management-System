                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 s">
                            <h2 class="pt-3 pb-4 pl-1 font-bold font-up deep-purple-text">Take Student <span>Attendence</span></h2>
                        </div>              
                    </div>
...
if(isset($_POST['att']))
{
	...
	$q="select id from addstudent where branch='$branch' and year='$year'";
	...
	
	{
		...
		$q2="select * from satt where sid='$a' and date=curdate()";
		...
		$q1="insert into satt(sid,date,status) values('$a',curdate(),'0')";
	...
	$result1=mysqli_query($con,$q1);
		...
		foreach($arr as $ar){
			...
			if($ar == $a)
			{
				...
				$q1="update satt set status='1' where sid='$ar' and date=curdate()";
...
echo "<script>alert('Attendence Submit..');</script>";
...
$sql="select* from addstudent where branch='$branch' and year='$year'";
    $result=mysqli_query($con,$sql);

    ?>
<form action="?branch=<?php echo $branch; ?>&year=<?php echo $year; ?>" method="post">
<table class="table table-striped"><tr><th>Roll No.</th><th>Name</th><th>Gender</th><th>Branch</th><th>Year</th><th>Present</th></tr>
    ...
    <?php
        ...
        {
        echo "<tr><td>".$row['id']."</td><td>".$row['sname']."</td><td>".$row['gender']."</td><td>".$row['branch']."</td><td>".$row['year']."</td><td><input type='checkbox' name='att[]' value=".$row['id']."></td></tr>";
...
?>
					...
					<div style="padding:10px;padding-top:10px"><input type="submit" class="btn btn-info s" name="submitat" style="width:10%;"/></div>
