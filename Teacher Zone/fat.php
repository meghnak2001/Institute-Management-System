...
<head>
    ...
    <title>Teacher/View Attendence</title>
...
{
	$id=$_SESSION["teacher"];
	$sql="select a.*, b.* from tatt a, addteacher b where a.tid = b.id and b.userid='$id'";
	$result=mysqli_query($con,$sql);
	echo "<table id='example' class='table table-striped table-bordered' style='width:100%;margin-top:10px;'>
	<thead><tr><th>S.No.</th><th>Name</th><th>Roll No.</th><th>Date</th><th>Status</th></thead>";
		
		while($row=mysqli_fetch_assoc($result))
		{
			
		echo "	<tr><td>".$row['aid']."</td><td>".$row['name']."</td><td>".$row['tid']."</td><td>".$row['date']."</td><td>";
